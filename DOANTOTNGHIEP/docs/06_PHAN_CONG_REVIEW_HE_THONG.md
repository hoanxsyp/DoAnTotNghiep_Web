...
|---|---|---|
| `user_profiles` | 1-1 với `users` (`uk_user_profiles_user_id`); N-1 tùy chọn tới `provinces`/`districts` | `date_of_birth`, `bio VARCHAR(500)`, `occupation`, `address_detail`, `preferred_gender_requirement` |
...
**Repository**: `ListingRepository` (`JpaRepository` + `JpaSpecificationExecutor`), `ListingImageRepository`, `ListingAmenityRepository`, `ListingEditHistoryRepository`. Không dùng `@SQLRestriction` cho xóa mềm — mọi truy vấn công khai/chủ sở hữu bắt buộc qua `findAliveById` (có `deletedAt IS NULL`); `findAnyById` chỉ dành cho Admin/Moderator (`LISTING_VIEW_ANY`).
...
6. **DB**: `listingRepository.save(listing)` (INSERT `listings`), `replaceAmenities` (INSERT `listing_amenities`), `recordHistory` (INSERT `listing_edit_histories` dòng `__create__`).
...
| `listing_edit_histories` | Append-only (không có `updated_at`/xóa mềm) | N-1 `listings`, N-1 `users` (editor_id, SET NULL khi user bị xóa) | `field_name`, `old_value/new_value` (TEXT), `is_sensitive_change`, `status_before/status_after`, `edit_batch_id` (CHAR 36, nhóm các thay đổi cùng 1 lần sửa) |
...
- **`ListingEditHistory.fieldName` bị cắt cứng 50 ký tự** (`field.length() > 50 ? field.substring(0, 50) : field`) — nếu sau này thêm field có tên dài hơn 50 ký tự sẽ bị cắt, khó truy vết trong lịch sử.
...
- □ Kiểm tra DB — sau mỗi hành động UI, đối chiếu trực tiếp bảng `listings`, `listing_edit_histories`, `moderation_actions`, `audit_logs`, `recommendation_logs`, `prediction_histories`, `search_histories` (đặc biệt xác nhận bảng này có thực sự luôn rỗng như nghi vấn hay không) để chắc chắn dữ liệu ghi đúng, đủ, không thiếu vết.
