# RoomFinder — Module Chatbot AI hỗ trợ tìm phòng trọ (Backend)

Cài đặt theo `SPEC_Module_Chatbot_AI_Tim_Phong_Tro.md`: chatbot tiếng Việt biến câu nói
...
Đọc theo thứ tự tùy nhu cầu:

| File | Vai trò | Khi nào đọc |
|---|---|---|
| **`README.md`** (file này) | Cách chạy, kiến trúc tóm tắt, ánh xạ code ↔ SPEC | Bắt đầu ở đây |
| **`FLOW_DE_HIEU.md`** | Giải thích **dễ hiểu** (ví von + hội thoại mẫu 5 lượt), gồm cả GĐ2/GĐ3 | Muốn nắm ý tưởng nhanh, không cần code |
| **`FLOW_XU_LY.md`** | Flow **kỹ thuật** một lượt chat (GĐ1→GĐ3), đối chiếu `ChatOrchestrator` | Trước khi đọc/sửa code |
...
3. Build & run (ở thư mục gốc repo — nơi có `pom.xml`):

```bash
mvn spring-boot:run
```

App chạy ở http://localhost:8080 (cấu hình `server.port` trong `application.yml`).

> **Chưa cài Maven?** Hai cách không cần cài gì:
> - **Docker** (mục 3) — build Maven chạy bên trong container.
> - **IDE**: mở thư mục gốc repo bằng IntelliJ IDEA / VS Code (Extension Pack for Java);
...
Repo phẳng: backend Spring Boot nằm ngay ở **thư mục gốc** (không có thư mục con
`backend/`); các module phụ trợ (`frontend/`, `ml/`, `nlu-service/`) nằm cạnh bên.
