...
# Bộ test theo từng chế độ. GOLD = data THẬT held-out (chỉ có nhãn intent — data
# hội thoại data2.md không phải bài đăng tìm phòng nên KHÔNG có nhãn NER). PROXY =
# bộ tay viết cũ (còn dùng cho NER/Slot vì chưa có NER thật). Xem README/data2.md.
DATASETS = {
    "proxy": {"intent": "intent_test_real.jsonl", "ner": "ner_test_real.jsonl"},
    "gold": {"intent": "intent_test_gold.jsonl"},
}
...
def run_side(side, args):
    ...
    # Cache proxy giữ tên cũ (`{side}_intent.jsonl`); gold thêm hậu tố để không đè.
    suffix = "" if args.dataset == "proxy" else f"_{args.dataset}"
    for dataset, fname in DATASETS[args.dataset].items():
        rows = load_jsonl(DATA / fname)
        cache = OUT / f"{side}_{dataset}{suffix}.jsonl"
        done = {}
        if cache.exists():
...
def intent_metrics(recs):
    ...
    # Macro-F1 trên các nhãn CÓ XUẤT HIỆN (gold ∪ pred) — cùng quy ước với
    # sklearn/evaluate lúc train (không phạt các nhãn 0-mẫu như calculate_cost trên
    # GOLD). Accuracy vẫn là số dẫn; macro-F1 chỉ tham chiếu.
...
def gold_intent_section():
    """Mục 1 — Intent trên GOLD thật (28 câu held-out). Chỉ intent: GOLD không có
    nhãn NER nên Slot/Span-F1 nằm ở mục PROXY."""
    gold_n = len(load_jsonl(DATA / DATASETS["gold"]["intent"]))
    lines = [
        f"## 1. Intent trên GOLD thật ({gold_n} câu held-out) — bộ đánh giá CHÍNH",
        "",
        "| Phương án | Intent Acc | Intent Macro-F1 |",
        "|---|---|---|",
    ]
    for side, label in SIDES:
        fi = OUT / f"{side}_intent_gold.jsonl"
        if not fi.exists():
            lines.append(f"| {label} | chưa chạy | chưa chạy |")
            continue
        acc, mf1 = intent_metrics(load_jsonl(fi))
        lines.append(f"| {label} | {pct(acc)} | {pct(mf1)} |")
    lines += [
        "",
        f"- Test set: `data/{DATASETS['gold']['intent']}` — {gold_n} câu DỮ LIỆU THẬT "
        "held-out (data.md + data2.md), tách bởi `data/split_real_data.py`.",
        "- GOLD chỉ có 6/8 intent (thiếu `calculate_cost`, `policy_inquiry` — 0 mẫu). "
        "Macro-F1 tính trên các nhãn có xuất hiện (cùng quy ước sklearn/evaluate lúc "
        "train). **Accuracy là số dẫn**; macro-F1 chỉ tham chiếu vì GOLD còn nhỏ.",
    ]
    return lines


def proxy_section():
    """Mục 2 — Slot/Span-F1 + Latency + Chi phí trên bộ PROXY (NER chưa có data thật)."""
    lines = [
    ...
    for side, label in SIDES:
        ...
        if side == "phobert":
            ...
            cost = (f"~{usd * USD_VND:,.0f}đ (~${usd:.3f}; {ti:.0f} in + {to:.0f} out tok/msg)"
                    .replace(",", "."))
        lines.append(f"| {label} | {pct(acc)} | {pct(sf1)} | {pct(spf1)} "
...
def report(args):
    lines = ["# So sánh NLU — SPEC §14.4", ""]
    lines += gold_intent_section()
    lines += [""]
    lines += proxy_section()
    text = "\n".join(lines)
    OUT.mkdir(exist_ok=True)
